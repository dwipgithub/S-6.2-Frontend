...
const RL311 = () => {
    ...
    const getDataRLTigaTitikSebelas = async (event) => {
        setSpinner(true)
        try {
            const customConfig = {
                headers: {
            ...
            const results = await axiosJWT.get('/apisirs/rlTigaTitikSebelas',
                customConfig)

            const rlTigaTitikSebelasDetails = results.data.data.map((value) => {
                return value.rl_tiga_titik_sebelas_details
            })
            ...
            let sortedProducts = dataRLTigaTitikSebelasDetails.sort((p1, p2) =>
            p1.jenis_pelayanan.no > p2.jenis_pelayanan.no 
              ? 1
              : p1.jenis_pelayanan.no < p2.jenis_pelayanan.no
              ? -1
              : 0
    ...
    const getRLTigaTitikSebelasTemplate = async() => {
        try {
            const response = await axiosJWT.get('/apisirs/jeniskpelayanan?rlid=11', {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
    ...
    const Cari = async (e) => {
        ...
        try {
           ...
           // console.log(results)
            ...
            setDataRL(datarlTigaTitikSebelasDetails)
           // console.log(datarlTigaTitikSebelasDetails)
           // console.log(dataRL)
        } catch (error) {
            console.log(error)
        }
    ...
    const hapusData = async(id) => {
        ...
        try {
            const results = await axiosJWT.delete(`/apisirs/rlTigaTitikSebelasdetail/${id}`,
                customConfig)
            setDataRL((current) =>
