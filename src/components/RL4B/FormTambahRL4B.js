...
const FormTambahRL4B = () => {
  ...
  const CariPenyakit = async (e) => {
    ...
    try {
      const response = await axiosJWT.get(
        "/apisirs/rlempatb/penyakit?search=" + caripenyakit,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const DetailPenyakit = async (id) => {
    setSpinner(true)
    try {
      const response = await axiosJWT.get("/apisirs/rlempatb/idpenyakit?id=" + id, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const SIMPAN = async (e) => {
    ...
    if(e.target[20].value >= counttotal){
      try {
        const customConfig = {
          headers: {
        ...
        await axiosJWT.post(
          "/apisirs/rlempatb",
          {
            ...
            data: [
              {
                jenisGolId: parseInt(e.target[1].value),
    ...
    <div className="container" style={{ marginTop: "70px" }}>
      ...
      <div className="row mt-3">
        ...
        {/* <Link to={`/rl4b`} style={{ textDecoration: "none" }}>
          ...
          <span style={{ color: "gray" }}>INPUT DATA RL 4B</span>
        </Link> */}
        <div className="col-md-6">
            ...
            <div className="card">
              <div className="card-body">
                  <form onSubmit={SIMPAN}>
                    <h5 className="card-title h5">
                      Input Data Penyakit {datainput[0].namaPenyakit}
                    </h5>
