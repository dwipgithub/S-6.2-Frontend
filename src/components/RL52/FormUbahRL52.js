...
export const FormUbahRL52 = () => {
    ...
    const changeHandler = (event, index) => {
        ...
        if (name === 'jumlahLakiLaki') {
            if(event.target.value === ''){
                    
                event.target.value = 0
                event.target.select(event.target.value)
                }
            setJumlahLakiLaki(parseInt(event.target.value))
            setJumlah(parseInt(event.target.value) + parseInt(jumlahPerempuan))
            ...
            setJumlahPerempuan(parseInt(event.target.value))
            setJumlah(parseInt(event.target.value) + parseInt(jumlahLakiLaki))
        }
    }
    ...
    const getRLLimaTitikDuaById = async () => {
        ...
        setJumlahLakiLaki(response.data.data.jumlahLakiLaki);
                        ...
                        <table className={style.rlTable}>
                            ...
                            <tbody>
                                <tr key={id}>
                                    ...
                                    <td>
                                        <input type="number" min="0" onFocus={handleFocus} maxLength={7} onInput={(e) => maxLengthCheck(e)}  name="jumlahLakiLaki" className="form-control" value={jumlahLakiLaki} 
                                            onChange={e => changeHandler(e)} onPaste={preventPasteNegative} onKeyPress={preventMinus} />
                                    </td>
                                    <td>
                                        <input type="number" min="0" onFocus={handleFocus} maxLength={7} onInput={(e) => maxLengthCheck(e)}  name="jumlahPerempuan" className="form-control" value={jumlahPerempuan} 
                                            onChange={e => changeHandler(e)} onPaste={preventPasteNegative} onKeyPress={preventMinus} />
                                    </td>
                                    <td>
